...
head(merged_dat)
PH148_total <- subset(merged_dat, tree_ID=="PH148")
dim(merged_dat)
dim(PH148_total)

#PH148$year <- str_sub(PH148$date_census,1,4)

PH148_total$month <- str_sub(PH148_total$date_census,6,7)

PH148_total$year <- str_sub(PH148_total$date_census,1,4)

PH148 <- subset(PH148_total, year %in% c(2021))
...
# young leaves
yl_PH148_sum_dat <- PH148 %>% group_by(month) 

# mature leaves
ml_PH148_sum_dat <- PH148 %>% group_by(month)

# old leaves
ol_PH148_sum_dat <- PH148 %>% group_by(month)

# flower bud
fb_PH148_sum_dat <- PH148 %>% group_by(month)

# open flower
fo_PH148_sum_dat <- PH148 %>% group_by(month)

# unripe fruit
fu_PH148_sum_dat <- PH148 %>% group_by(month)

# ripe fruit
fr_PH148_sum_dat <- PH148 %>% group_by(month)
...
#young leaves
yl_PH148_avg <- yl_PH148_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH148_avg <- fo_PH148_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH148_avg <- fu_PH148_sum_dat %>% group_by(month) %>% 
...
yl_PH148_avg$pheno_metric = "young_leaf"
fo_PH148_avg$pheno_metric = "open_flower"
fu_PH148_avg$pheno_metric = "unripe_fruit"

PH148_combined = rbind(yl_PH148_avg, fo_PH148_avg, fu_PH148_avg)

PH148_combined_plot <- ggplot() +
  # geom_bar(data=PH148_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH148_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH148_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH148_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH148 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH148_combined.pdf")
