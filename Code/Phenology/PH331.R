...
head(merged_dat)
PH331_total <- subset(merged_dat, tree_ID=="PH331")
dim(merged_dat)
dim(PH331_total)

#PH331$year <- str_sub(PH331$date_census,1,4)

PH331_total$month <- str_sub(PH331_total$date_census,6,7)

PH331_total$year <- str_sub(PH331_total$date_census,1,4)

PH331 <- subset(PH331_total, year %in% c(2021))
...
# young leaves
yl_PH331_sum_dat <- PH331 %>% group_by(month) 

# mature leaves
ml_PH331_sum_dat <- PH331 %>% group_by(month)

# old leaves
ol_PH331_sum_dat <- PH331 %>% group_by(month)

# flower bud
fb_PH331_sum_dat <- PH331 %>% group_by(month)

# open flower
fo_PH331_sum_dat <- PH331 %>% group_by(month)

# unripe fruit
fu_PH331_sum_dat <- PH331 %>% group_by(month)

# ripe fruit
fr_PH331_sum_dat <- PH331 %>% group_by(month)
...
#young leaves
yl_PH331_avg <- yl_PH331_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH331_avg <- fo_PH331_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH331_avg <- fu_PH331_sum_dat %>% group_by(month) %>% 
...
yl_PH331_avg$pheno_metric = "young_leaf"
fo_PH331_avg$pheno_metric = "open_flower"
fu_PH331_avg$pheno_metric = "unripe_fruit"

PH331_combined = rbind(yl_PH331_avg, fo_PH331_avg, fu_PH331_avg)

PH331_combined_plot <- ggplot() +
  # geom_bar(data=PH331_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH331_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH331_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH331_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH331 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH331_combined.pdf")
