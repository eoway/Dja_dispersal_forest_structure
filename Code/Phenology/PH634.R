...
head(merged_dat)
PH634_total <- subset(merged_dat, tree_ID=="PH634")
dim(merged_dat)
dim(PH634_total)

#PH634$year <- str_sub(PH634$date_census,1,4)

PH634_total$month <- str_sub(PH634_total$date_census,6,7)

PH634_total$year <- str_sub(PH634_total$date_census,1,4)

PH634 <- subset(PH634_total, year %in% c(2021))
...
# young leaves
yl_PH634_sum_dat <- PH634 %>% group_by(month) 

# mature leaves
ml_PH634_sum_dat <- PH634 %>% group_by(month)

# old leaves
ol_PH634_sum_dat <- PH634 %>% group_by(month)

# flower bud
fb_PH634_sum_dat <- PH634 %>% group_by(month)

# open flower
fo_PH634_sum_dat <- PH634 %>% group_by(month)

# unripe fruit
fu_PH634_sum_dat <- PH634 %>% group_by(month)

# ripe fruit
fr_PH634_sum_dat <- PH634 %>% group_by(month)
...
#young leaves
yl_PH634_avg <- yl_PH634_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH634_avg <- fo_PH634_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH634_avg <- fu_PH634_sum_dat %>% group_by(month) %>% 
...
yl_PH634_avg$pheno_metric = "young_leaf"
fo_PH634_avg$pheno_metric = "open_flower"
fu_PH634_avg$pheno_metric = "unripe_fruit"

PH634_combined = rbind(yl_PH634_avg, fo_PH634_avg, fu_PH634_avg)

PH634_combined_plot <- ggplot() +
  # geom_bar(data=PH634_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH634_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH634_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH634_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH634 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH634_combined.pdf")
