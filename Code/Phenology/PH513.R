...
head(merged_dat)
PH513 <- subset(merged_dat, tree_ID=="PH513")
dim(merged_dat)
dim(PH513)

PH513$year <- str_sub(PH513$date_census,1,4)

PH513$month <- str_sub(PH513$date_census,6,7)
...
# young leaves
yl_PH513_sum_dat <- PH513 %>% group_by(month, year) 

# mature leaves
ml_PH513_sum_dat <- PH513 %>% group_by(month, year)

# old leaves
ol_PH513_sum_dat <- PH513 %>% group_by(month, year)

# flower bud
fb_PH513_sum_dat <- PH513 %>% group_by(month, year)

# open flower
fo_PH513_sum_dat <- PH513 %>% group_by(month, year)

# unripe fruit
fu_PH513_sum_dat <- PH513 %>% group_by(month, year)

# ripe fruit
fr_PH513_sum_dat <- PH513 %>% group_by(month, year)
...
#young leaves
yl_PH513_avg <- yl_PH513_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH513_avg <- fo_PH513_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH513_avg <- fu_PH513_sum_dat %>% group_by(month) %>% 
...
yl_PH513_avg$pheno_metric = "young_leaf"
fo_PH513_avg$pheno_metric = "open_flower"
fu_PH513_avg$pheno_metric = "unripe_fruit"

PH513_combined = rbind(yl_PH513_avg, fo_PH513_avg, fu_PH513_avg)

PH513_combined_plot <- ggplot() +
  # geom_bar(data=PH513_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH513_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH513_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH513_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH513 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH513_combined.pdf")
