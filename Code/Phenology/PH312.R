...
head(merged_dat)
PH312_total <- subset(merged_dat, tree_ID=="PH312")
dim(merged_dat)
dim(PH312_total)

#PH312$year <- str_sub(PH312$date_census,1,4)

PH312_total$month <- str_sub(PH312_total$date_census,6,7)

PH312_total$year <- str_sub(PH312_total$date_census,1,4)

PH312 <- subset(PH312_total, year %in% c(2021))

# import total weather data
weather_total <- read_csv("weather_data/24Oct2017-17Fev2022_Bouamir_Weather_data.csv")
...
# young leaves
yl_PH312_sum_dat <- PH312 %>% group_by(month) 

# mature leaves
ml_PH312_sum_dat <- PH312 %>% group_by(month)

# old leaves
ol_PH312_sum_dat <- PH312 %>% group_by(month)

# flower bud
fb_PH312_sum_dat <- PH312 %>% group_by(month)

# open flower
fo_PH312_sum_dat <- PH312 %>% group_by(month)

# unripe fruit
fu_PH312_sum_dat <- PH312 %>% group_by(month)

# ripe fruit
fr_PH312_sum_dat <- PH312 %>% group_by(month)
...
#young leaves
yl_PH312_avg <- yl_PH312_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH312_avg <- fo_PH312_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH312_avg <- fu_PH312_sum_dat %>% group_by(month) %>% 
...
yl_PH312_avg$pheno_metric = "young_leaf"
fo_PH312_avg$pheno_metric = "open_flower"
fu_PH312_avg$pheno_metric = "unripe_fruit"

PH312_combined = rbind(yl_PH312_avg, fo_PH312_avg, fu_PH312_avg)

PH312_combined_plot <- ggplot() +
  # geom_bar(data=PH312_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH312_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH312_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH312_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  scale_y_continuous("mean % intensity", breaks=c(0,3,6,9,12), labels=c("0","0-25","25-50","50-75","75-100"), 
                     sec.axis = sec_axis(~ . *1, name = "rainfall (mm)")) +
  scale_x_discrete("month", labels=c("J","F","M","A","M","J","J","A","S","O","N","D")) +
  labs(title="Phenological Change over Time in PH312 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH312_combined.pdf")
