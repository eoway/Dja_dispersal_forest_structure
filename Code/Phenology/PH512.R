...
head(merged_dat)
PH512 <- subset(merged_dat, tree_ID=="PH512")
dim(merged_dat)
dim(PH512)

PH512$year <- str_sub(PH512$date_census,1,4)

PH512$month <- str_sub(PH512$date_census,6,7)
...
# young leaves
yl_PH512_sum_dat <- PH512 %>% group_by(month, year) 

# mature leaves
ml_PH512_sum_dat <- PH512 %>% group_by(month, year)

# old leaves
ol_PH512_sum_dat <- PH512 %>% group_by(month, year)

# flower bud
fb_PH512_sum_dat <- PH512 %>% group_by(month, year)

# open flower
fo_PH512_sum_dat <- PH512 %>% group_by(month, year)

# unripe fruit
fu_PH512_sum_dat <- PH512 %>% group_by(month, year)

# ripe fruit
fr_PH512_sum_dat <- PH512 %>% group_by(month, year)
...
#young leaves
yl_PH512_avg <- yl_PH512_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH512_avg <- fo_PH512_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH512_avg <- fu_PH512_sum_dat %>% group_by(month) %>% 
...
yl_PH512_avg$pheno_metric = "young_leaf"
fo_PH512_avg$pheno_metric = "open_flower"
fu_PH512_avg$pheno_metric = "unripe_fruit"

PH512_combined = rbind(yl_PH512_avg, fo_PH512_avg, fu_PH512_avg)

PH512_combined_plot <- ggplot() +
  # geom_bar(data=PH512_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH512_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH512_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH512_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH512 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH512_combined.pdf")
