...
head(merged_dat)
PH641_total <- subset(merged_dat, tree_ID=="PH641")
dim(merged_dat)
dim(PH641_total)

#PH641$year <- str_sub(PH641$date_census,1,4)

PH641_total$month <- str_sub(PH641_total$date_census,6,7)

PH641_total$year <- str_sub(PH641_total$date_census,1,4)

PH641 <- subset(PH641_total, year %in% c(2021))
...
# young leaves
yl_PH641_sum_dat <- PH641 %>% group_by(month) 

# mature leaves
ml_PH641_sum_dat <- PH641 %>% group_by(month)

# old leaves
ol_PH641_sum_dat <- PH641 %>% group_by(month)

# flower bud
fb_PH641_sum_dat <- PH641 %>% group_by(month)

# open flower
fo_PH641_sum_dat <- PH641 %>% group_by(month)

# unripe fruit
fu_PH641_sum_dat <- PH641 %>% group_by(month)

# ripe fruit
fr_PH641_sum_dat <- PH641 %>% group_by(month)
...
#young leaves
yl_PH641_avg <- yl_PH641_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH641_avg <- fo_PH641_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH641_avg <- fu_PH641_sum_dat %>% group_by(month) %>% 
...
yl_PH641_avg$pheno_metric = "young_leaf"
fo_PH641_avg$pheno_metric = "open_flower"
fu_PH641_avg$pheno_metric = "unripe_fruit"

PH641_combined = rbind(yl_PH641_avg, fo_PH641_avg, fu_PH641_avg)

PH641_combined_plot <- ggplot() +
  # geom_bar(data=PH641_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH641_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH641_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH641_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH641 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH641_combined.pdf")
