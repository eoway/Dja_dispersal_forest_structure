...
head(merged_dat)
PH638_total <- subset(merged_dat, tree_ID=="PH638")
dim(merged_dat)
dim(PH638_total)

#PH638$year <- str_sub(PH638$date_census,1,4)

PH638_total$month <- str_sub(PH638_total$date_census,6,7)

PH638_total$year <- str_sub(PH638_total$date_census,1,4)

PH638 <- subset(PH638_total, year %in% c(2021))
...
# young leaves
yl_PH638_sum_dat <- PH638 %>% group_by(month) 

# mature leaves
ml_PH638_sum_dat <- PH638 %>% group_by(month)

# old leaves
ol_PH638_sum_dat <- PH638 %>% group_by(month)

# flower bud
fb_PH638_sum_dat <- PH638 %>% group_by(month)

# open flower
fo_PH638_sum_dat <- PH638 %>% group_by(month)

# unripe fruit
fu_PH638_sum_dat <- PH638 %>% group_by(month)

# ripe fruit
fr_PH638_sum_dat <- PH638 %>% group_by(month)
...
#young leaves
yl_PH638_avg <- yl_PH638_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH638_avg <- fo_PH638_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH638_avg <- fu_PH638_sum_dat %>% group_by(month) %>% 
...
yl_PH638_avg$pheno_metric = "young_leaf"
fo_PH638_avg$pheno_metric = "open_flower"
fu_PH638_avg$pheno_metric = "unripe_fruit"

PH638_combined = rbind(yl_PH638_avg, fo_PH638_avg, fu_PH638_avg)

PH638_combined_plot <- ggplot() +
  # geom_bar(data=PH638_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH638_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH638_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH638_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH638 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH638_combined.pdf")
