...
head(merged_dat)
PH362_total <- subset(merged_dat, tree_ID=="PH362")
dim(merged_dat)
dim(PH362_total)

#PH362$year <- str_sub(PH362$date_census,1,4)

PH362_total$month <- str_sub(PH362_total$date_census,6,7)

PH362_total$year <- str_sub(PH362_total$date_census,1,4)

PH362 <- subset(PH362_total, year %in% c(2021))
...
# young leaves
yl_PH362_sum_dat <- PH362 %>% group_by(month) 

# mature leaves
ml_PH362_sum_dat <- PH362 %>% group_by(month)

# old leaves
ol_PH362_sum_dat <- PH362 %>% group_by(month)

# flower bud
fb_PH362_sum_dat <- PH362 %>% group_by(month)

# open flower
fo_PH362_sum_dat <- PH362 %>% group_by(month)

# unripe fruit
fu_PH362_sum_dat <- PH362 %>% group_by(month)

# ripe fruit
fr_PH362_sum_dat <- PH362 %>% group_by(month)
...
#young leaves
yl_PH362_avg <- yl_PH362_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH362_avg <- fo_PH362_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH362_avg <- fu_PH362_sum_dat %>% group_by(month) %>% 
...
yl_PH362_avg$pheno_metric = "young_leaf"
fo_PH362_avg$pheno_metric = "open_flower"
fu_PH362_avg$pheno_metric = "unripe_fruit"

PH362_combined = rbind(yl_PH362_avg, fo_PH362_avg, fu_PH362_avg)

PH362_combined_plot <- ggplot() +
  # geom_bar(data=PH362_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH362_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH362_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH362_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH362 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH362_combined.pdf")
