...
head(merged_dat)
PH359 <- subset(merged_dat, tree_ID=="PH359")
dim(merged_dat)
dim(PH359)

PH359$year <- str_sub(PH359$date_census,1,4)

PH359$month <- str_sub(PH359$date_census,6,7)
...
# young leaves
yl_PH359_sum_dat <- PH359 %>% group_by(month, year) 

# mature leaves
ml_PH359_sum_dat <- PH359 %>% group_by(month, year)

# old leaves
ol_PH359_sum_dat <- PH359 %>% group_by(month, year)

# flower bud
fb_PH359_sum_dat <- PH359 %>% group_by(month, year)

# open flower
fo_PH359_sum_dat <- PH359 %>% group_by(month, year)

# unripe fruit
fu_PH359_sum_dat <- PH359 %>% group_by(month, year)

# ripe fruit
fr_PH359_sum_dat <- PH359 %>% group_by(month, year)
...
#young leaves
yl_PH359_avg <- yl_PH359_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH359_avg <- fo_PH359_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH359_avg <- fu_PH359_sum_dat %>% group_by(month) %>% 
...
yl_PH359_avg$pheno_metric = "young_leaf"
fo_PH359_avg$pheno_metric = "open_flower"
fu_PH359_avg$pheno_metric = "unripe_fruit"

PH359_combined = rbind(yl_PH359_avg, fo_PH359_avg, fu_PH359_avg)

PH359_combined_plot <- ggplot() +
  # geom_bar(data=PH359_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH359_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH359_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH359_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH359 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH359_combined.pdf")
