...
head(merged_dat)
PH440_total <- subset(merged_dat, tree_ID=="PH440")
dim(merged_dat)
dim(PH440_total)

#PH440$year <- str_sub(PH440$date_census,1,4)

PH440_total$month <- str_sub(PH440_total$date_census,6,7)

PH440_total$year <- str_sub(PH440_total$date_census,1,4)

PH440 <- subset(PH440_total, year %in% c(2021))
...
# young leaves
yl_PH440_sum_dat <- PH440 %>% group_by(month) 

# mature leaves
ml_PH440_sum_dat <- PH440 %>% group_by(month)

# old leaves
ol_PH440_sum_dat <- PH440 %>% group_by(month)

# flower bud
fb_PH440_sum_dat <- PH440 %>% group_by(month)

# open flower
fo_PH440_sum_dat <- PH440 %>% group_by(month)

# unripe fruit
fu_PH440_sum_dat <- PH440 %>% group_by(month)

# ripe fruit
fr_PH440_sum_dat <- PH440 %>% group_by(month)
...
#young leaves
yl_PH440_avg <- yl_PH440_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH440_avg <- fo_PH440_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH440_avg <- fu_PH440_sum_dat %>% group_by(month) %>% 
...
yl_PH440_avg$pheno_metric = "young_leaf"
fo_PH440_avg$pheno_metric = "open_flower"
fu_PH440_avg$pheno_metric = "unripe_fruit"

PH440_combined = rbind(yl_PH440_avg, fo_PH440_avg, fu_PH440_avg)

PH440_combined_plot <- ggplot() +
  # geom_bar(data=PH440_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH440_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH440_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH440_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH440 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH440_combined.pdf")
