...
head(merged_dat)
PH626_total <- subset(merged_dat, tree_ID=="PH626")
dim(merged_dat)
dim(PH626_total)

#PH626$year <- str_sub(PH626$date_census,1,4)

PH626_total$month <- str_sub(PH626_total$date_census,6,7)

PH626_total$year <- str_sub(PH626_total$date_census,1,4)

PH626 <- subset(PH626_total, year %in% c(2021))
...
# young leaves
yl_PH626_sum_dat <- PH626 %>% group_by(month) 

# mature leaves
ml_PH626_sum_dat <- PH626 %>% group_by(month)

# old leaves
ol_PH626_sum_dat <- PH626 %>% group_by(month)

# flower bud
fb_PH626_sum_dat <- PH626 %>% group_by(month)

# open flower
fo_PH626_sum_dat <- PH626 %>% group_by(month)

# unripe fruit
fu_PH626_sum_dat <- PH626 %>% group_by(month)

# ripe fruit
fr_PH626_sum_dat <- PH626 %>% group_by(month)
...
#young leaves
yl_PH626_avg <- yl_PH626_sum_dat %>% group_by(month) %>% 
...
#open flowers
fo_PH626_avg <- fo_PH626_sum_dat %>% group_by(month) %>% 
...
#unripe fruit
fu_PH626_avg <- fu_PH626_sum_dat %>% group_by(month) %>% 
...
yl_PH626_avg$pheno_metric = "young_leaf"
fo_PH626_avg$pheno_metric = "open_flower"
fu_PH626_avg$pheno_metric = "unripe_fruit"

PH626_combined = rbind(yl_PH626_avg, fo_PH626_avg, fu_PH626_avg)

PH626_combined_plot <- ggplot() +
  # geom_bar(data=PH626_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
  #          stat='identity', position=position_dodge()) +
  geom_errorbar(data=PH626_combined, aes(x=month, group=as.factor(pheno_metric), 
                                         ymin = total_avg*3-abs(sd*3), ymax = total_avg*3+abs(sd*3)), 
                width = 0.2, position=position_dodge(0.3))+
  geom_point(data=PH626_combined, aes(x=month, y=total_avg * 3, fill=as.factor(pheno_metric)), 
             pch=21, size=4, position=position_dodge(0.3)) +
  geom_line(data=PH626_combined, aes(x=month, y=total_avg * 3, group=as.factor(pheno_metric)), linetype="dashed",
  ...
  labs(title="Combined Phenological Change over Time in PH626 Tree", fill="year") +
...
ggsave("analysis_plots/specific_trees/PH626_combined.pdf")
